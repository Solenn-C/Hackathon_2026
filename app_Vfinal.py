...
def get_map_data(value, column_name):
    ...
    return pd.DataFrame({'code_dep': dep_codes, column_name: [value * (1 + np.random.uniform(-0.02, 0.02)) for _ in dep_codes]})
...
with st.sidebar:
    ...
    echelle = st.selectbox("Échelle d'analyse", ["National", "Régional"], key="sb_echelle")
...
with tab1:
    ...
    with col_desc1:
        ...
        st.caption("Température annuelle moyenne relevée sur le territoire français, exprimée en degrés Celsius (°C).")
        
        st.write("**co2_cumule (Stock de CO2)**")
        st.caption("Somme totale des émissions de dioxyde de carbone accumulées dans l'atmosphère depuis 1990.")
        
        st.write("**empreinte_co2 (Consommation individuelle)**")
        st.caption("Émissions de CO2 par habitant liées au mode de vie (transport, alimentation, chauffage).")
        
        st.write("**glacier_cumule (Fonte des Glaces)**")
        st.caption("Indicateur de la perte d'épaisseur des glaciers de montagne en mètres cumulés.")

        st.write("**sea_level (Niveau de la Mer)**")
        st.caption("Élévation du niveau moyen des océans en millimètres (mm) par rapport à la référence de 1990.")

    with col_desc2:
        st.write("**impact_incendies (Coût Économique)**")
        st.caption("Estimation des pertes financières et des dégâts matériels causés par les incendies de forêt chaque année.")
        
        st.write("**nb_catastrophes (Événements Extrêmes)**")
        st.caption("Fréquence annuelle des catastrophes naturelles enregistrées (tempêtes, inondations, crues).")
        
        st.write("**date_vendange (Indicateur Thermique)**")
        st.caption("Écart de la date de récolte par rapport à la normale. Une date plus précoce témoigne d'étés plus chauds.")
        
        st.write("**swi_score (Indice d'Humidité des Sols)**")
        st.caption("Le 'Soil Water Index' mesure l'état de sécheresse des sols. Un score faible indique un stress hydrique pour la végétation.")

    st.info("💡 Ces indicateurs permettent à notre IA de calculer un score de risque global pour les projections futures.")
...
with tab2:
    ...
    try:
        ...
        with c_f1: annee_choisie = st.select_slider("Explorer la chronologie :", options=years, value=years[-1])
        with c_f2: ind_sel = st.selectbox("Indicateur clé :", indicators)
        ...
        with col_left:
            ...
            fig_map = px.choropleth_mapbox(
                df_map, geojson=geojson_france, locations='code_dep', featureidkey="properties.code",
                color=ind_sel, color_continuous_scale="YlOrRd", mapbox_style="carto-positron",
                zoom=4.5, center={"lat": 46.2, "lon": 2.2}, opacity=0.7
            )
        ...
        with col_right:
            ...
            fig_trend = px.line(df_all, x='annee', y=ind_sel, markers=True, color_discrete_sequence=['#1f77b4'])
        ...
        with st.expander("🔍 Analyses scientifiques des corrélations"):
            ...
            with ca2:
                ...
                st.plotly_chart(px.area(df_all, x="annee", y="glacier_cumule", title=None), use_container_width=True)


    except: st.error("⚠️ Erreur : Backend non connecté ou données inaccessibles.")
...
with tab3:
    ...
    with col_p1:
        ...
        scenario = st.selectbox("Scénario d'émissions", ["optimiste", "intermediaire", "pessimiste"], index=1, key="proj_scenario")
        
        try:
            res_p = requests.get(f"{API_URL}/projections", params={"scenario": scenario}).json()
            years_p = res_p["years"]
            target_year = st.select_slider("Année cible :", options=years_p, value=2050, key="proj_year")
            
            # Calcul des métriques de projection
            temp_p = res_p["values"][years_p.index(target_year)]
            delta = temp_p - 13.06 # Référence 2024
            
            st.metric("Température Prédite", f"{temp_p:.2f} °C", f"{delta:+.2f} °C", delta_color="inverse")
            st.info(f"En {target_year}, le modèle prévoit une augmentation de {delta:.1f}°C par rapport à aujourd'hui.")
        except:
    ...
    with col_p2:
        ...
        try:
            ...
            res_m = requests.get(f"{API_URL}/map_projections", params={"scenario": scenario, "year": target_year}).json()
            ...
            fig_mock = px.choropleth_mapbox(
                mock_data, geojson=geojson_france, locations='code_dep', featureidkey="properties.code",
                color='score', color_continuous_scale="OrRd", mapbox_style="carto-positron",
                zoom=4.8, center={"lat": 46.2, "lon": 2.2}, opacity=0.8
            )
