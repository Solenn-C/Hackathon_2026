"""Consolidation des séries mensuelles homogénéisées de température minimale (TN).

Lit tous les CSV du dossier SH_TN_metropole, extrait les métadonnées de l'en-tête,
...
BRONZE_DIR = Path("data/bronze/SH_TN_metropole")
SILVER_DIR = Path("data/silver")
OUTPUT_FILE = SILVER_DIR / "tn_metropole.parquet"
...
def parse_csv(path: Path) -> pd.DataFrame | None:
    """Parse un fichier CSV homogénéisé de TN et retourne un DataFrame enrichi.
    ...
    Returns:
        DataFrame avec les colonnes [date, tn, q_hom, num_poste, nom_usuel,
    ...
    # Parse des données tabulaires
    if not data_lines:
        logger.warning("Fichier vide (pas de données) : %s", path.name)
        return None
    ...
    df = df.rename(columns={"yyyymm": "date_ym", "valeur": "tn", "q_hom": "q_hom"})
    df["date"] = pd.to_datetime(df["date_ym"].astype(str), format="%Y%m")
    df = df.drop(columns=["date_ym"])

    # Conversion : les valeurs sont en 1/10 de °C
    df["tn"] = pd.to_numeric(df["tn"], errors="coerce")

    # Ajout des métadonnées station
...
def main() -> None:
    """Point d'entrée : consolide tous les CSV TN en un seul fichier Parquet."""
