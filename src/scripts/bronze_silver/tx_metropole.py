"""Consolidation des séries mensuelles homogénéisées de température maximale (TX).

Lit tous les CSV du dossier SH_TX_metropole, extrait les métadonnées de l'en-tête,
...
BRONZE_DIR = Path("data/bronze/SH_TX_metropole")
SILVER_DIR = Path("data/silver")
OUTPUT_FILE = SILVER_DIR / "tx_metropole.parquet"
...
def parse_csv(path: Path) -> pd.DataFrame | None:
    """Parse un fichier CSV homogénéisé de TX et retourne un DataFrame enrichi.
    ...
    Returns:
        DataFrame avec les colonnes [date, tx, q_hom, num_poste, nom_usuel,
    ...
    df = df.rename(columns={"yyyymm": "date_ym", "valeur": "tx", "q_hom": "q_hom"})
...
def main() -> None:
    """Point d'entrée : consolide tous les CSV TX en un seul fichier Parquet."""
